...
func init() {
	tmplFuncs := template.FuncMap{
		...
		"collectStandardImportsInMessage":       collectStandardImportsInMessage,
		"collectExtraImports":                   collectExtraImports,
		"collectExtraImportsInMessage":          collectExtraImportsInMessage,
...
{{ define "setters_with_import" }}
{{- $transportPkg := .TransportPackage -}}
{{ range .Fields }}
// Set{{ .Name }} sets {{ .Name }}, Tag {{ .Tag }}.
func ({{ template "receiver" }} {{ $.Name }}) {{ if .IsGroup }}Set{{ .Name }}(f {{ $transportPkg }}.{{ .Name }}RepeatingGroup){
	{{ template "receiver" }}.SetGroup(f)
}{{ else }}{{ template "fieldsetter" . }}{{ end }}
{{ end }}{{ end }}
...
{{ define "getters_with_import" }}
{{- $transportPkg := .TransportPackage -}}
{{ range .Fields }}
// Get{{ .Name }} gets {{ .Name }}, Tag {{ .Tag }}.
func ({{ template "receiver" }} {{ $.Name }}) {{if .IsGroup}}Get{{ .Name }}() ({{ $transportPkg }}.{{ .Name }}RepeatingGroup, quickfix.MessageRejectError) {
	f := {{ $transportPkg }}.New{{ .Name }}RepeatingGroup()
	err := {{ template "receiver" }}.GetGroup(f)
	return f, err
}{{ else }}{{ template "fieldvaluegetter" .}}{{ end }}
{{ end }}{{ end }}
...
import (
	{{- if collectStandardImportsInMessage .MessageDef }}
	{{- range collectStandardImportsInMessage .MessageDef }}
	"{{ . }}"
	{{- end }}{{ "\n" }}
	{{- end }}
	{{- if collectExtraImportsInMessage .MessageDef }}
	{{- range collectExtraImportsInMessage .MessageDef }}
	"{{ . }}"
	{{- end }}{{ "\n" }}
	{{- end }}
	"github.com/quickfixgo/quickfix"
...
{{ template "setters_with_import" . }}
{{ template "getters_with_import" . }}
{{ template "hasers" . }}
	`))

	ComponentsTemplate = template.Must(template.Must(baseTemplate.Clone()).Parse(`
{{ define "receiver" }}c{{ end }}
// Code generated by quickfix. DO NOT EDIT.
package {{ .Package }}
